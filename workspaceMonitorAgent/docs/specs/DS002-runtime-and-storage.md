...
summary: Defines the manifest, supervisor, private metrics stream, SQLite WAL store, and retention boundary.
---
...
The collector must request /api/edge/workspace-metrics?follow=1 with a private Router assertion and consume newline-delimited snapshots. SQLite must use the resource_samples table, WAL mode, a five-second busy timeout, and a thirteen-month retention window. The legacy resource_exceedances table is migrated into resource_samples and is not a second active store.
