...
Follow-up invariant after the OnlyOffice JWT drift incident: implemented. The Document Server is now owned by the Ploinky-managed `onlyOffice` agent (`onlyOffice/manifest.json` + `onlyOffice/scripts/hooks/preinstall.sh`), pinned to Docker tag `9.3.1` by default, with required `JWT_SECRET` derived from the `AchillesIDE/explorer/ONLYOFFICE_JWT_SECRET` identity so it equals Explorer's required `ONLYOFFICE_JWT_SECRET` without any cross-agent coordination. The manifest explicitly runs Document Server via `entrypoint: "/bin/bash"` and `start: "/app/ds/run-document-server.sh"` and publishes port `80` on `127.0.0.1:8082` (`prod`/`default`) or `127.0.0.1:18082` (`dev`). Persistent storage for `/var/log/onlyoffice`, `/var/www/onlyoffice/Data`, and `/var/lib/onlyoffice` is bind-mounted under `.ploinky/data/onlyOffice/`; internal PostgreSQL, RabbitMQ, and Redis image data directories are left container-owned so rootless Podman ownership mapping cannot block service startup. Explorer is added as a dependent (`enable: ["onlyOffice", ...]` in `explorer/manifest.json`); Explorer's preinstall hook no longer creates the raw `ploinky_onlyoffice_<workspace>` sidecar. The `onlyOffice` agent's preinstall first verifies that the target image is locally available or pullable, then removes any leftover legacy sidecar before binding host port `8082`; pull or removal failure exits non-zero so the migration cannot delete the old service and continue into a broken replacement. The durable spec is `onlyOffice/docs/specs/DS01-ploinky-agent-invariant.md`.

Follow-up invariant for extra Ploinky mounts: implemented. Manifest-declared extra host mounts must live under the workspace `.ploinky/` tree; durable service data belongs under `.ploinky/data/<agent-or-service>/...`, and generated runtime inputs belong under `.ploinky/agents/<agent>/...`. WebMeet application state now mounts from `.ploinky/data/webmeetAgent/data` to `/data`, recording files mount from `.ploinky/data/webmeet/recordings` to `/data/recordings`, and generated LiveKit/Egress configs mount into `/working-data/generated/...` instead of `/code`.
...
Root cause at the time: OnlyOffice Document Server was still running as a raw host-managed Podman sidecar named `ploinky_onlyoffice_explorerWorkspace`, created by Explorer's host preinstall hook. Because it was not a Ploinky agent, Ploinky injected the derived `ONLYOFFICE_JWT_SECRET` into Explorer but did not own/inject the sidecar's `JWT_SECRET`. The first repair aligned the derivation label; the second repair taught the host hook to derive the same value from `PLOINKY_MASTER_KEY` when `PLOINKY_DERIVED_MASTER_KEY` was not yet available in the host preinstall context.
...
That deploy was a functional repair, not the final architecture. The follow-up implementation now moves Document Server lifecycle ownership into the real `onlyOffice` Ploinky agent. The old `ploinky_onlyoffice_<workspace>` sidecar is only a migration source that the `onlyOffice` preinstall hook removes after the target image preflight succeeds; it is no longer compliant steady state. The deploy workflow now checks `api.js` through the internal URL after Ploinky TCP readiness and through the browser-visible public URL when `ONLYOFFICE_PUBLIC_URL` is configured. Post-migration validation must also prove, without printing secrets, that `ploinky status` shows the managed `onlyOffice` agent, Explorer and Document Server JWT secrets compare equal, and `/Confidential/My Space/test.doc` still flows through `Explorer -> Ploinky router -> dpuAgent`.
