...
Explorer's `default` profile must use `basic/web-publishing` as the public topology owner. Local, QA, and production launches must consume that same Explorer profile wiring instead of defining Explorer-specific `qa` or `prod` profile blocks. Web Publishing must run as a blocking startup config provider before Explorer's public-facing dependencies consume OnlyOffice or WebMeet public URLs, hostnames, callback bases, or TURN realms. Deployment workflows must clear stale direct public-topology workspace vars before startup and must not route Explorer deployment profiles through the standalone `basic/cloudflared` agent.
...
Manifest `openPorts` declarations are also outer Ploinky-box publication claims. They must use stable nonzero box-side ports so the active dependency graph can be validated before runtime mutation. OnlyOffice therefore maps its protected control listener as `127.0.0.1:17002:7000` in every profile, leaving LiveKit on `17000`; its editor and unpublished loopback-storage planes remain separate contracts.
