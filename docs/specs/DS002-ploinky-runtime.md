...
The tracked Explorer QA deployment must derive the canonical achillesAgentLib URL and immutable commit from the selected Ploinky runtime's dependency lock. It must reject a noncanonical source, a malformed commit, or a locked commit that differs from the selected remote default-branch head; select that branch with fail-closed branch fallback while retaining explicit per-repository graph branches; and never set a retired AgentLib selection environment variable. After all expected runtimes are admitted, deployment evidence must execute Ploinky's graph attestation and prove that the core plus every expected agent share the locked commit, content fingerprint, physical source identity, package hash, entry-point hashes, and stable read-only source root.
