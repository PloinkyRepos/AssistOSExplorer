...
Ploinky router authentication and secure invocation data are authoritative for protected browser and agent calls. Explorer declares SSO mode and its UserPersisto provider in its manifest; Ploinky binds that provider without hard-coding its identity in router code. The router must remotely revalidate SSO sessions and require the manifest-declared `explorer.access` capability before forwarding protected Explorer traffic. An agent must derive the acting principal from verified invocation context and must not trust a client-declared actor identity.
