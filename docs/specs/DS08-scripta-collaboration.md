...
Pure variant and voting behavior lives in `explorer/shared/document/scripta-state.js`. The reusable tabs, score, reaction, proposal, and optional editing surface lives in `explorer/shared/ui/scripta-variants-view/` and is served through Explorer's existing `/shared/*` whitelist. The shared view uses class-scoped neutral layout containers rather than globally styled page-level elements, so it retains the same visual contract in Advanced Editor and WebMeet. Image options are cloned from a component-owned inert template, ordinary Close, Replace, and Delete clicks use WebSkel `data-local-action` presenter methods, and the tab panel permits the contextual menu to overflow without clipping while its tab rail remains the dedicated horizontal scroller. The Advanced Editor plugin and WebMeet provide persistence adapters to that component; the shared component never writes document or room state itself.
...
Variant image mutations are canonical WebMeet events before Explorer executes them. The event audit exposes stable image ordinals and safe layout properties, never Explorer asset, image, variant, paragraph, or resource identifiers. File selection and media validation remain UI/Explorer responsibilities; the LLM may reference only an existing Blackboard image or an ordinal already present in the focused SCRIPTA variant.

The same internal boundary exposes `webmeet_media_commit` and `webmeet_media_get` only to the WebMeet agent. Committed room images live at `document-multimedia/webmeet/<roomId>/assets/<assetId>.<ext>`. Images belong to individual paragraph variants, beside that variant's text; they are not shared paragraph metadata and are not modeled as standalone image paragraphs. The canonical mutations are `p-variant-image-insert`, `p-variant-image-replace`, and `p-variant-image-delete`, with the same ownership rule as variant text edits. WebMeet sends the authorized room id and asset id, while Explorer resolves the authoritative workspace URL and serializes only the winning variant's images as ordinary Markdown inside that paragraph. WebMeet never sends arbitrary media paths and never writes the Markdown file directly.
