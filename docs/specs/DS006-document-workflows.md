...
The Markdown CRDT is the collaboration mechanism for workspace Markdown source editing and structured SCRIPTA operations. Its private state is stored under `.data/explorer/automerge/documents/<document-id>.automerge`; the Markdown path is not the document identity. Office co-editing uses OnlyOffice's separate collaboration transport and signed callback persistence. Public SCRIPTA replicas must expose only the sanitized collaboration model and must not leak author-only or reaction-private metadata.
