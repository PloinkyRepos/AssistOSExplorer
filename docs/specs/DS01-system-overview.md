...
New WebMeet attachment assets use the canonical room-scoped layout `document-multimedia/webmeet/<roomId>/assets/`. Recognized images are stored as `<assetId>.<image-ext>`, while every non-image is stored as `<assetId>.bin` so the authenticated inline workspace route cannot execute active file types. The adjacent `<assetId>.metadata.json` record remains distinct from uploaded content, including JSON files. The generic `/blobs/explorer` route is staging only: an internal WebMeet media commit verifies the blob reference against its sidecar metadata, enforces the 15 MB limit, recognizes PNG, JPEG, WebP, or GIF by content signature, and moves the staged blob into the final asset path. Recognized images retain intrinsic dimensions; all other files retain their sanitized original filename, extension, MIME type, and size for display and download. Chat and Blackboard reference the final asset, while supported image insertion into SCRIPTA does not create another copy.
...
The Markdown source toolbar must provide functional link and image actions rather than exposing TinyMDE syntax placeholders as complete workflows. Link insertion uses an Explorer modal with label, destination, optional title, inline validation, and a Markdown preview; native browser prompts are not part of this flow. Local image insertion accepts PNG, JPEG, WebP, or GIF up to 20 MB, stores the file in a unique `<document>.assets/` directory beside the Markdown document, and inserts a relative Markdown image reference through the normal editor/CRDT change path. SVG upload is rejected. Dragged image files use the same upload path. Confidential DPU Markdown does not silently copy images into the public workspace; its image action remains unavailable until the DPU provides an equivalent attachment contract.

SOPLang-aware Markdown editing must remain an explicit action. When the user selects `Advanced edit`, Explorer may hydrate the selected Markdown file through the document runtime and open the structured document UI for Achilles metadata, chapters, paragraphs, commands, variables, references, media tags, and related document-node workflows. This keeps ordinary Markdown editing lightweight while preserving the structured document capability for files that need it.
...
Explorer must also revalidate the router session when its document is restored through browser Back navigation or the back-forward cache. A `401` session probe replaces the restored protected page with `/auth/login` while preserving the Explorer URL in `returnTo`; a valid session leaves other users and tabs unchanged, and an indeterminate network or server failure must not be treated as logout.
