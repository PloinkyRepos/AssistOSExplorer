...
The `workspace-monitor` application plugin is mounted in `file-exp:account-menu` only for administrators. It opens `#workspace-monitor-dashboard` in a new browser tab. The page contains Overview, Resources, Router Logs, Policy Audit, and DPU Audit tabs. Overview combines live resource values with persisted threshold-exceedance history and administrator-editable persistence thresholds.

The UI uses Explorer theme variables and shared controls. It does not expose lifecycle, restart, shell-command, agent configuration, plugin configuration, or Copilot controls.
...
The agent stores data in a SQLite database under its Ploinky persistent storage root using the `node:sqlite` implementation included with Node 24. The database uses WAL mode and retains 13 months of samples. Every available metric is written at most once every ten seconds. Existing exceedance-only records are migrated into the complete sample table, but historical values that were never collected cannot be reconstructed.

The four reference thresholds are Agents CPU, Agents memory, Router CPU, and Router memory. They draw comparison lines and are stored with each sample; they do not control persistence. The agent derives Agents by summing the runtime metrics and consumes Ploinky's existing `total` value, whose contract already includes both runtime and Router usage. No Ploinky payload extension is required. Defaults are 80%, 4 GiB, 80%, and 512 MiB respectively. The persisted `workspace.*` series names remain stable for compatibility and contain the Agents aggregate.
...
- `workspace_monitor_history_query` accepts ISO `from` and `to`, optional series selection, and a maximum point count capped at 50,000. For each time bucket it returns the maximum recorded value, the threshold recorded with that maximum, and the latest recorded threshold. The first threshold classifies the peak; the latest threshold draws the threshold timeline.
