...
The configured default registration role must exist and must not grant administrative capabilities. SSO router access is capability-based. For an existing local-password workspace, Explorer explicitly declares `routerAccess.localAuthRoles: [admin, user]`; this compatibility mapping applies only to runtime-verified local authentication and cannot be selected by a supplied token field, an SSO identity, or a guest. Explorer's protected surface requires `explorer.access`; therefore both `admin` and `user` can enter Explorer, while `selfRegistered` cannot unless an administrator changes policy.
