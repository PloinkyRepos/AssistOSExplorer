...
| Build remote env file | Writes `PLOINKY_MASTER_KEY`, supported `WEB_PUBLISHING_*` inputs, supported WebMeet runtime values, and the Ploinky Node image tag to `/tmp/explorer_qa_env`. Blank optional values are omitted, except `PLOINKY_MASTER_KEY`. | [`.github/workflows/deploy-explorer-qa.yml`](../.github/workflows/deploy-explorer-qa.yml) |
...
| Start Explorer | Runs `env PLOINKY_MASTER_KEY="$PLOINKY_MASTER_KEY" "$PLOINKY" start AchillesIDE/explorer "$ROUTER_PORT" --profile "${PLOINKY_PROFILE:-prod}" ...`. Ploinky starts `basic/web-publishing`, runs its startup config provider, writes public topology values, and then resolves dependent agent env under the explicitly selected profile. | [`.github/workflows/deploy-explorer-qa.yml`](../.github/workflows/deploy-explorer-qa.yml) |
| Verify provider outputs | Reads `ONLYOFFICE_PUBLIC_URL` and `WEBMEET_PUBLIC_LIVEKIT_URL` with `ploinky echo` after startup. Signaling must equal exactly `wss://meet.<WEB_PUBLISHING_BASE_DOMAIN>`; a missing, altered-host, or downgraded-scheme value fails. The runner probes the canonical HTTPS hostname with a bounded, bodyless HTTP/1.1 WebSocket-upgrade request on exact `/rtc` and requires the expected unauthenticated `401`, then requires browser-visible OnlyOffice `api.js` through `ONLYOFFICE_PUBLIC_URL`. It deliberately does not probe the container-local `ONLYOFFICE_INTERNAL_URL` from the host. | [`.github/workflows/deploy-explorer-qa.yml`](../.github/workflows/deploy-explorer-qa.yml) |
...
The QA workflow passes `PLOINKY_PROFILE` (default `prod`) explicitly to `ploinky start --profile`, but
