...
gh secret set SOUL_GATEWAY_PROVIDER_API_KEY --repo AssistOS-AI/AssistOSExplorer
```
...
`SOUL_GATEWAY_PROVIDER_API_KEY` is the credential used by Explorer's local Soul Gateway to register the remote `soul.axiologic.dev` gateway as the normal `soul-gateway` provider. The workflow also accepts the legacy GitHub secret name `SOUL_GATEWAY_API_KEY` as a fallback and maps it to `SOUL_GATEWAY_PROVIDER_API_KEY`. Local Ploinky starts do the same through the Soul Gateway manifest: an operator `SOUL_GATEWAY_API_KEY` can source the provider credential, but it does not replace Explorer's generated local `SOUL_GATEWAY_API_KEY`.

Explorer helper agents must keep their `SOUL_GATEWAY_API_KEY` generated by Ploinky. That generated key authenticates calls into the local Soul Gateway router service. The remote production gateway is just another provider inside that local gateway, like OpenAI or Anthropic.

`LMSTUDIO_PROXY_TOKEN` is only needed for deployments that leave `SOUL_GATEWAY_PROVIDER_API_KEY` unset and use Soul Gateway's local LLM provider bootstrap. The deploy workflow stores it as `LOCAL_LLM_API_KEY` through `ploinky var` so Soul Gateway can create an encrypted provider account at startup. Use a `LOCAL_LLM_API_KEY` secret instead if the endpoint token is not an LM Studio proxy token:

```sh
gh secret set LMSTUDIO_PROXY_TOKEN --repo AssistOS-AI/AssistOSExplorer
gh secret set LOCAL_LLM_API_KEY --repo AssistOS-AI/AssistOSExplorer
```
...
The deploy workflow does not synthesize local LLM defaults when `SOUL_GATEWAY_PROVIDER_API_KEY` is supplied; production registers `soul.axiologic.dev` as the local gateway's `soul-gateway` provider. Optional provider variables can override the default base URL, model discovery behavior, or mirrored aliases:

```sh
gh variable set SOUL_GATEWAY_PROVIDER_BASE_URL --repo AssistOS-AI/AssistOSExplorer --body https://soul.axiologic.dev/v1
gh variable set SOUL_GATEWAY_PROVIDER_DISCOVERY_MODE --repo AssistOS-AI/AssistOSExplorer --body auto
gh variable set SOUL_GATEWAY_PROVIDER_ALIASES --repo AssistOS-AI/AssistOSExplorer --body fast,axl/fast,plan,code,write,deep,ultra
```

`SOUL_GATEWAY_PROVIDER_DISCOVERY_MODE=auto` syncs the remote gateway's `/models` response at local gateway startup. The alias list mirrors same-named discovered remote gateway models, so local calls to `fast`, `plan`, or `code` can route through local aliases to provider-backed models such as `soul-gateway/fast`.

The local LLM fallback is only for deployments without the remote Soul Gateway provider credential:

```sh
gh variable set LOCAL_LLM_BASE_URL --repo AssistOS-AI/AssistOSExplorer --body https://lmstudio.axiologic.dev/v1
gh variable set LOCAL_LLM_MODEL --repo AssistOS-AI/AssistOSExplorer --body gemma-3-12b-it
gh variable set LOCAL_LLM_DISCOVERY_MODE --repo AssistOS-AI/AssistOSExplorer --body single
```

`LOCAL_LLM_DISCOVERY_MODE=single` avoids exposing LM Studio models that are installed but not currently loaded. Use `auto` only for endpoints that can reliably serve every model returned by `/models`.
...
For manual deployments that should register the remote Soul Gateway provider, set the provider credential or the legacy source name. Both become the remote provider account while the local gateway key remains generated:

```sh
ploinky var SOUL_GATEWAY_PROVIDER_API_KEY "$REMOTE_SOUL_GATEWAY_API_KEY"
# or
ploinky var SOUL_GATEWAY_API_KEY "$REMOTE_SOUL_GATEWAY_API_KEY"
```

Do not make Explorer helper agents opt into explicit `SOUL_GATEWAY_API_KEY` overrides. Their generated gateway credentials ignore explicit overrides, so remote keys do not replace the local gateway key.
