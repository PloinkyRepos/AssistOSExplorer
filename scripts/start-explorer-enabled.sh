...
has_static_config() {
  node -e "try{const fs=require('fs');const d=JSON.parse(fs.readFileSync('.ploinky/agents','utf8'));process.exit(d&&d._config&&d._config.static&&d._config.static.agent?0:1)}catch(e){process.exit(1)}"
}
