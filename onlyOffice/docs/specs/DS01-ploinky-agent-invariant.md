...
The manifest must publish the protected control listener on container port `7000` through stable loopback box-side port `17002` (`127.0.0.1:17002:7000`) so the Ploinky `httpServices` route targets `/control/*` and the outer Ploinky box can declare the same stable boundary socket. The former WebMeet `17000` health publication is retired in favor of private readiness scripts. The browser-facing editor proxy on container port `8080` remains a separate host port for the OnlyOffice editor assets and `/doc/*` WebSocket paths. The loopback storage listener on `9100` must not be published.

Published ports: the control listener uses fixed loopback box-side port `17002` (`127.0.0.1:17002:7000`), while the editor proxy publishes a separate fixed host port (`127.0.0.1:8082:8080`, dev `127.0.0.1:18082:8080`). Every profile must keep a nonzero stable box-side port so graph publication planning can validate conflicts before either the outer box or nested agent runtime changes. Ploinky manifest `openPorts` values are literal and are not env-interpolated, so a collision requires an intentional manifest/topology change rather than an ephemeral port assignment.
