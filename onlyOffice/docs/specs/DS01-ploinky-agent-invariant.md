...
the bundled support services, the root-level wrapper resolves exactly one
packaged PostgreSQL cluster and prepares only its three guarded regular
...
log directory and four init-script log files with the package-required owners
and modes. Symbolic links, hard-linked files, ambiguous clusters, missing
required paths, unexpected path shapes, or ownership and mode drift after
preparation fail closed before service startup. A targeted
