...
Controlled shutdown must stop new control and editor admission, force-save each writable session, and await its durable callback acknowledgement before stopping DocumentServer. The wrapper and its foreground helpers must run in a dedicated process group so one SIGTERM reaches the complete foreground tree. After the authoritative application drain, the wrapper may notify DocumentServer's loopback cluster-inactive endpoint, but the notification must have a two-second total timeout and must not extend the agent beyond Ploinky's clean-exit bound.
