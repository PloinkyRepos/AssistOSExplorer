...
test('Workspace Monitor keeps read-only concerns in separate tabs', () => {
    ...
    assert.match(script, /appendChartSeries\(svg, totalSamples, scale, 'total'\)/);
    assert.match(script, /appendChartSeries\(svg, agentsSamples, scale, 'agents'\)/);
    ...
    assert.match(script, /comparisonThresholds = item\.valueThresholds\.length/);
    ...
    assert.match(script, /timestamp - previousTimestamp > stepMs \* 1\.5/);
