...
function passkeyOrigin() {
    if (!isLocalIpHost()) return window.location.origin;
...
export const authPreferenceController = {
    ...
    renderPreferredAuthMethods() {
        ...
        if (!this.state.preferredAuthStatusType) {
    ...
    async loadPreferredAuthProfile() {
        ...
        try {
            const payload = await callUserPersistoTool("userpersisto_get_my_auth_profile");
    ...
    async savePreferredAuthMethod() {
        const preferredAuthMethod = String(this.preferredAuthSelectEl?.value || "").trim();
        if (!preferredAuthMethod || this.state.authPreferenceBusy) return;
        this.state.authPreferenceBusy = true;
        this.state.preferredAuthStatus = "Saving preferred sign-in method...";
        this.state.preferredAuthStatusType = "";
        this.renderPreferredAuthStatus();
        this.renderPreferredAuthMethods();
        try {
            const payload = await callUserPersistoTool("userpersisto_set_my_preferred_auth_method", { preferredAuthMethod });
            this.state.authUser = payload.user || this.state.authUser;
            this.state.preferredAuthStatus = "Preferred sign-in method saved.";
            this.state.preferredAuthStatusType = "";
        } catch (error) {
            this.state.preferredAuthStatus = error?.message || "Failed to save preferred sign-in method.";
            this.state.preferredAuthStatusType = "error";
        } finally {
            this.state.authPreferenceBusy = false;
            this.renderPreferredAuthMethods();
    ...
    async setupMyPasskey() {
        ...
        this.renderPreferredAuthMethods();
        try {
            const start = await callUserPersistoTool("userpersisto_start_my_passkey_registration", {
            ...
            await callUserPersistoTool("userpersisto_verify_my_passkey_registration", {
                credential: credentialToServer(credential)
            });
            this.state.preferredAuthStatus = "Passkey added.";
            this.state.preferredAuthStatusType = "";
            await showToast("Passkey added.", "success");
