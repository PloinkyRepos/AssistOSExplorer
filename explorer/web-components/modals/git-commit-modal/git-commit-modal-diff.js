...
export function createGitCommitDiff(ctx) {
    ...
    const renderDiff = (text, { filePath, section, loading = false, isError = false } = {}) => {
    ...
    const loadDiffForSelection = async () => {
        ...
        if (cached) {
            renderDiff(cached, { filePath, section });
            return;
        }

        renderDiff('Loading diff...', { filePath, section, loading: true });
        try {
            ...
            renderDiff(diffText, { filePath, section });
        } catch (error) {
            const message = normalizeErrorMessage(error);
            renderDiff(message, { filePath, section, isError: true });
        }
    };
