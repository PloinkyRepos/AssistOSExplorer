...
export class ChapterItem {
    constructor(element, invalidate) {
        ...
        const refreshChapterFn = this.refreshChapter.bind(this);
        this.refreshChapter = async () => {
            this.chapter = await refreshChapterFn(this._document.id, this.chapter.id);
        };
    ...
    async insertNewParagraph(paragraphId, position) {
        ...
        if (existingIndex === -1) {
            ...
            this.chapter.paragraphs[existingIndex] = newParagraph;
        }
        ...
        let previousParagraph = previousParagraphId
            ? this.element.querySelector(`paragraph-item[data-paragraph-id="${previousParagraphId}"]`)
            : null;
