...
export class UserpersistoAccountMenu {
    ...
    async refreshProfile() {
        ...
        this.renderState();
        try {
            const payload = await callUserPersisto('userpersisto_get_my_auth_profile');
            this.state.user = payload.user || null;
            this.state.authMethods = Array.isArray(payload.authMethods) ? payload.authMethods : [];
            this.state.enrollments = payload.enrollments && typeof payload.enrollments === 'object' ? payload.enrollments : {};
        } catch (_) {
    ...
    async changePreferredAuthMethod(target) {
        ...
        try {
            const payload = await callUserPersisto('userpersisto_set_my_preferred_auth_method', { preferredAuthMethod });
            this.state.user = payload.user || this.state.user;
            this.renderState();
        } catch (error) {
            await this.refreshProfile();
