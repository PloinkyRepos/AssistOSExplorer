...
- Verifies runtime prompt includes the `Conversation History (last 10 replies)` section.
...
- Verifies session history is loaded as a bounded context excerpt (latest 10 user/agent messages).
- Verifies deterministic fallback when no prior history exists.
...
- Verifies history continues to be persisted on disk while orchestration context remains profile-memory-first.
