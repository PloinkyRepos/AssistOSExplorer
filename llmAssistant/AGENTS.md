# llmAssistant Agent Guide

## Scope

llmAssistant provides bounded LLM helper operations for Explorer and peer-agent workflows without exposing provider-specific contracts to callers.
...
Runs `scripts/startAgent.sh` in `node:20-alpine` with `lite-sandbox: true`.
...
- `node --check scripts/startAgent.sh`
- `node --check lib/*.mjs`
