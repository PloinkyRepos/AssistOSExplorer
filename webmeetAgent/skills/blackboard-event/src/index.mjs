...
export async function action({ promptText, llmAgent, context }) {
    ...
    const prompt = [
        ...
        'Connections are line widgets with properties.connection.from/to. Each endpoint is independent: null means a free endpoint stored by properties.line; an attached endpoint has exactly one of widgetId, groupId, or command-local ref plus anchor left, right, top, or bottom. A groupId must come from context board.groups. Connections never cross workspaces.',
