...
The prompt is the complete instruction. Runtime context contains only the safe semantic `board` projection. Every ungrouped widget and every rigid group has one transient global `ordinal` matching the number shown by the browser while the command runs. Group badges use `G<number>` and resolve through `board.groups`; grouped members are semantic detail and cannot be targeted individually. In compact or speech-transcribed commands, a spoken or numeric integer immediately after a widget kind is interpreted as that ungrouped widget's ordinal when it matches an existing target, even if punctuation or words such as `by` are omitted. If no ordinal identifies a target, `focusedGroupId` takes precedence over a compatible focused widget. A SCRIPTA widget additionally exposes a safe document outline and its active chapter/paragraph focus, without paragraph content or participant-private data. Context never contains viewport, zoom, scroll, paths, participant-private data, audit identifiers, or a board revision.
...
Widgets sharing a non-empty `groupId` are one rigid block and one ordinal target. Existing groups are targeted with `{ "type": "group", "groupId": "..." }`; their canonical update uses `payload.patch.transform` with `translation`, `resize`, or `rotationDelta`. Group deletion removes every member, while `ungroup` preserves current geometry. A group id must always come from `board.groups` context.
