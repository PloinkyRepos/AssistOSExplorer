...
| --- | --- | --- |
| WebMeet room record | Durable application object with a `room_<uuid>` id, title, `roomType`, derived `roomName`, status, wrapped data-encryption key, and encrypted payload. | `/data/rooms/*.json`, backed by `.ploinky/data/webmeetAgent/data/rooms/*.json`. |
