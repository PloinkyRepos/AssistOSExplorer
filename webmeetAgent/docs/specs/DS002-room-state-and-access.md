...
The encrypted WebMeet payload contains members, chat messages, resources, AI agent metadata, blackboard state, and event snapshots. Blackboard state is stored as the authoritative final room object under the room payload and contains serialized widgets, widget properties, visibility, locked state, versions, and the bounded undo/redo history needed for active room edits. Runtime-only browser fields such as DOM nodes, local selection, viewport, and drag previews are not persisted. Room payloads use AES-256-GCM with per-room data keys wrapped by `PLOINKY_WEBMEET_MASTER_KEY`. That key is an agent-scoped generated secret declared with `generatedSecret: true`. The store does not support automatic legacy key fallback.
