...
Validated chat and toolbar uploads create editable `image` widgets backed by Explorer media assets. The widget stores only safe source metadata, intrinsic dimensions, alt text, style, and Blackboard geometry; move, resize, rotate, grouping, deletion, history, and focus therefore use the normal canonical event path. Chat publication creates the attachment and widget atomically after Explorer accepts the staged bytes. Selecting a chat thumbnail opens the current participant's local Blackboard surface and selects the corresponding widget without changing peers' layouts. Contextual insertion of an image into SCRIPTA reuses its asset. Contextual or `/robo` insertion of a group uses the existing origin-clean browser renderer, commits the resulting PNG, and invokes the canonical SCRIPTA media mutation.
