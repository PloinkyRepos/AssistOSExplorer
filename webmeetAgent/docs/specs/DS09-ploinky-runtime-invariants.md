...
- Dependencies: LiveKit Node packages must be supplied through Ploinky's prepared dependency cache mounted at `/code/node_modules` and `/Agent/node_modules`. The service runtime must fail with a clear cache-rebuild instruction when the cache is missing rather than running `npm install` inside the long-lived container.
...
### Question #3: Why does `webmeetAgent` refuse runtime dependency installation?

Response:
LiveKit agent packages include native runtime assets that must match the target container ABI. Ploinky prepares that dependency tree before launch and mounts it read-only into the service container, which keeps startup deterministic and avoids long-running package-manager work inside the runtime process. If the cache is missing or stale, Ploinky should rebuild it before the agent starts.
