...
Chat image publication stages raw bytes through Explorer's existing blob route, then calls `webmeet_image_publish` with the returned Ploinky blob reference (`id`, `agent`, and logical `localPath`). WebMeet authorizes the participant and mutable room before asking Explorer to validate that reference and resolve its logical path through Explorer's configured workspace. PNG, JPEG, WebP, and GIF are accepted up to 15 MB and are identified by content signature rather than the browser MIME header. The final file is `document-multimedia/webmeet/<roomId>/assets/<assetId>.<ext>`. One serialized room mutation appends the chat attachment, creates the editable image widget, records one Blackboard history command, updates focus, and emits both chat and Blackboard invalidations. A failed room authorization never commits the staged blob. A committed asset is referenced by stable asset id and workspace URL rather than duplicated into chat or Blackboard state.
