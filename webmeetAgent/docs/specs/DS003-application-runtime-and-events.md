...
The room blackboard has the same authoritative control-plane rule. `webmeet_blackboard_get` is the read path and `webmeet_event_command` is the sole mutation path. The command accepts canonical JSON, the deterministic `/event <allowlisted-action> [JSON object]` shorthand, or natural text from `/event` and `/robo`. The shorthand is parsed locally and never invokes the LLM; natural text uses the event interpreter. Interpretation has a bounded lifetime and a timeout finalizes the audit as an error instead of leaving it pending indefinitely. Browser UI actions produce the same canonical event contract as chat commands; there are no separate apply, undo, redo, or SCRIPTA mutation tools. Drag and typing previews remain local until release, blur, save, or submit. Every final event carries `eventId`, idempotent `commandId`, `expectedBoardVersion`, a board/widget target, an allowlisted action, and a bounded payload. The board version is checked inside the room mutation lock, so stale same-room commands fail with `version_conflict` while different rooms continue independently. Accepted changes are persisted in the encrypted room payload and announced through `blackboard.updated`. Blackboard visibility uses the realtime-only `blackboard.visibility_changed` event.
