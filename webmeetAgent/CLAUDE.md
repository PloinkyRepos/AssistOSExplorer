...
Runs `scripts/startAgent.sh`, the WebMeet API, public proxy, and AgentServer in `docker.io/assistos/ploinky-node:24-bookworm-tools` on the `webmeet` network. The optional self-hosted LiveKit AI worker is owned by the separate `webmeetLivekitAiAgent` Ploinky agent and is not part of the default Explorer dependency graph.
