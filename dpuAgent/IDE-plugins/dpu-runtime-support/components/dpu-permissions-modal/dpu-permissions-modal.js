...
const RESOURCE_ROLES = ['access', 'read', 'write'];
const ROLE_DESCRIPTIONS = Object.freeze({
    ...
    resource: {
        access: 'Can inspect the resource identity and access state.',
        read: 'Can inspect provider facts, provenance, and protected results.',
        write: 'Can update intended use and create permission proposals.'
    }
});
...
export class DpuPermissionsModal {
    ...
    getRoleOptions() {
        if (this.state.kind === 'secret') return SECRET_ROLES;
        if (this.state.kind === 'resource') return RESOURCE_ROLES;
        return CONFIDENTIAL_ROLES;
    }
    ...
    canManagePermissions() {
        return this.state.kind === 'resource' ? this.state.role === 'write' : this.isOwner();
    }
    ...
    renderRoleLegend() {
        ...
        const descriptionMap = ROLE_DESCRIPTIONS[this.state.kind] || ROLE_DESCRIPTIONS.confidential;
        const options = this.getRoleOptions();
    ...
    renderSummary() {
        ...
        const kindLabel = this.state.kind === 'secret'
            ? 'Secret'
            : this.state.kind === 'resource' ? 'Research resource' : 'Confidential';
    ...
    renderAcl() {
        ...
        const canManage = this.canManagePermissions();
        const canMutateExisting = canManage && this.state.kind !== 'resource';
            ...
            this.setStatus(
                (this.state.role === 'write' || this.state.role === 'write-access')
                    ...
                    : this.state.kind === 'resource'
                        ? 'Resource write permission is required to create a sharing proposal.'
    ...
    async loadData() {
        ...
        try {
            ...
            if (this.state.kind === 'secret') {
                ...
                const result = await callDpuTool('dpu_resource_get', { id: this.state.id });
                const resource = result.resource || {};
                this.state.ownerId = String(resource.ownerId || '').trim();
        ...
        try {
            if (this.state.kind === 'secret') {
                ...
                const confirmed = await assistOS.UI.showModal('confirm-action-modal', {
                    message: `Confirm ${proposal?.type || 'permission change'}?\n\n${effects}\n\nExpires: ${proposal?.expiresAt || 'unknown'}`
                }, true);
                if (!confirmed) {
                    await callDpuTool('dpu_action_reject', { id: proposal.id });
                    this.state.busy = false;
                    this.updateFormState();
                    this.setStatus('Permission proposal rejected.', '');
                    return;
                }
